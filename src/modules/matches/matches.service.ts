...
export class MatchesService {
	 ...
	 * @param data
	 */
	async updateStatusAndNotify(match: Match, status: MatchStatus, data: any = {}): Promise<void> {
		if (match.status === status) return;

		match.status = status;

		if (match.callbackUrl) {
			this.logger.log(`Notifying URL '${match.callbackUrl}' for status '${match.status} (${match._id})'`);

			try {
				await axios.post(`${match.callbackUrl}?status=${match.status}`, { ...match.toJSON(), ...data })
			} catch (error) {
				if (error.code === "ECONNREFUSED") {
					this.logger.warn(`Failed to connect callback URL "${match.callbackUrl}"`);
				} else {
	...
	static async getAvailableRegionProvider(region: string): Promise<string[]> {
		const res = await axios.get(`${config.lighthouse.host}/api/v1/providers/region/${region}`, {
			headers: {
	 ...
	 */
	static async sendServerCreateRequest(options: ServerRequestOptions): Promise<any> {
